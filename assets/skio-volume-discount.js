...
// scope variables
(() => {
  const selector = 'a[href="/checkout/"]'

  const checkoutHandler = async (event) => {
    if ((!window.skio_ran_tier_discount && event.type == 'skioVolumeDiscount') || (!window.skio_ran_tier_discount && (event.target.matches(selector) || event.target.parentNode.matches(selector)))){
      ...
      if (rule) {
        ...
        await fetch(`/discount/${rule.code}`)
      }
  ...
  document.addEventListener('skioVolumeDiscount', checkoutHandler);
