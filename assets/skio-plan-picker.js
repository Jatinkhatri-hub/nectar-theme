    ...
    try {
      ...
      const getSelectedSellingPlanGroupId = () => {
        try {
          ...
          if ($onetime?.checked) {
            ...
            $onetime.closest('[data-product-form]').querySelector('[data-add-to-cart-subscribe]').style.display = 'none';
            $onetime.closest('[data-product-form]').querySelector('[data-add-to-cart-one-time]').style.display = 'block';
            return null;
          }

          const $selectedSellingPlanGroup = $planPicker.querySelector(`${skio.selectors.sellingPlanGroup()}:checked`);
          // if ($selectedSellingPlanGroup.querySelector('[data-add-to-cart-one-time]') != 'undefined' && $selectedSellingPlanGroup.querySelector('[data-add-to-cart-one-time]') != null) {
          //   $selectedSellingPlanGroup.closest('[data-product-form]').querySelector('[data-add-to-cart-subscribe]').style.display = 'block';
          //   $selectedSellingPlanGroup.closest('[data-product-form]').querySelector('[data-add-to-cart-one-time]').style.display = 'none';
          // } else {
      ...
      const updatePrices = () => {
        ...
        skio.availableSellingPlanGroupIds.forEach(availableSellingPlanGroupId => {	
          ...
          if (assert($sellingPlanGroup, `No selling group element found with id "${availableSellingPlanGroupId}"`,{ messageType: 'info' })) return;	
          
            const $sellingPlans = $sellingPlanGroup.closest(skio.selectors.groupContainer).querySelector(skio.selectors.sellingPlans());
          ...
          if (skio.selectedSellingPlanGroupId) {
            
            if ($sellingPlanGroup.querySelector('[data-add-to-cart-subscribe]')) {
              $sellingPlanGroup.closest('[data-product-form]').querySelector('[skio-external-price]').innerHTML = price;
            } else {
              document.querySelectorAll('[skio-external-price]').forEach(el => el.innerHTML = price);
            }
            
            if (document.querySelector('.suffix-subscription')) {
              document.querySelector('.suffix-subscription').style.display = 'inline-block';
              document.querySelector('.suffix-one-time').style.display = 'none';  
            }
          }
        })	
        // One-time	
        ...
        if (!skio.selectedSellingPlanGroupId) {
          document.querySelectorAll('[skio-external-price]').forEach(el => el.innerHTML = onetimePriceFormatted);

          if (document.querySelector('.suffix-subscription')) {
            document.querySelector('.suffix-subscription').style.display = 'none';
            document.querySelector('.suffix-one-time').style.display = 'inline-block';  
          }
        }
      }
